...
public class MainActivity extends AppCompatActivity {
    ...
    private final float xCat = 350; // задание поля для координаты X
    private final float yCat = 700; // задание поля для координаты Y
            ...
            switch (motionEvent.getAction()) { // метод getAction() считывает состояние касания (нажатие/движение/отпускание)
                ...
                case MotionEvent.ACTION_MOVE: // движение
                    ...
                    if (x < (xCat + deltaCat) && x > (xCat - deltaCat) && y < (yCat + deltaCat) && y > (yCat - deltaCat)) { // если пользователь коснулся места нахождения кота
                        ...
                        toast.setGravity(Gravity.TOP,(int) xCat-500, (int) yCat); // задание позиции на экране (положение, смещение по оси Х, смещение по оси Y)
